...
impl fmt::Debug for FSecp256 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "[{:0x} {:0x} {:0x} {:0x} {:0x}]", self.v[0], self.v[1], self.v[2], self.v[3], self.v[4])
    }
}
...
impl FSecp256 {
    fn chain(&self) -> (FSecp256, FSecp256, FSecp256) {
        ...
        let x22 = x11.sqr().sqr().sqr().sqr().sqr().sqr().sqr().sqr().sqr().sqr().sqr().mul(&x11);
        let x44 = {
            let mut r = x22.clone();
            for _ in 0..22 { r = r.sqr(); }
            r
        }.mul( &x22 );

        let x88 = {
            let mut r = x44.clone();
            for _ in 0..44 { r = r.sqr(); }
            r
        }.mul( &x44 );

        let x176 = {
            let mut r = x88.clone();
            for _ in 0..88 { r = r.sqr(); }
            r
        }.mul( &x88 );
...
impl Fq for FSecp256 {
    ...
    fn rand(rng: &mut Rng) -> Self {
        ...
        let mut r = FSecp256 { v: [ r0, r1, r2, r3, r4  ] };
        r.normalize_weak();
        r
    }

    fn from_slice(r: &[u64]) -> FSecp256 {
        FSecp256 { v:[ r[0], r[1], r[2], r[3], r[4] ] }
    }
    ...
    fn add(&self, a: &FSecp256) -> FSecp256 {
        ...
        return FSecp256 { v:[self.v[0] + a.v[0],
            self.v[1] + a.v[1],
            self.v[2] + a.v[2],
            self.v[3] + a.v[3],
            self.v[4] + a.v[4],
    ...
    fn equals(&self, b: &FSecp256) -> bool {
        return self.v[0]==b.v[0] && self.v[1]==b.v[1] && self.v[2]==b.v[2] 
            && self.v[3]==b.v[3] && self.v[4]==b.v[4];
    }
...
// Zq for the order, field for the group
    fn mul(&self, b: &FSecp256) -> FSecp256 {
        ...
        let d = d + (a.v[0] as u128) * (b.v[4] as u128)
        ...
        let d = d + (a.v[1] as u128) * b.v[4] as u128
        ...
        let c = c + (a.v[0] as u128) * (b.v[1] as u128) +
        	(a.v[1] as u128) * (b.v[0] as u128);
        ...
        let d = d + (a.v[2] as u128) * b.v[4] as u128
        ...
        let c = c + (a.v[0] as u128 * b.v[2] as u128)
        ...
        let d = d + (a.v[3] as u128 * b.v[4] as u128)
           + (a.v[4] as u128 * b.v[3] as u128);
        debug_assert!(d>>114 == 0);
        // println!("d7: {:x}", d);
    ...
    //      */
        ...
        debug_assert!( c>>112==0 );
        /* [d t4+(tx<<48) t3 0 0 c] = [p8 0 0 0 p4 p3 0 0 p0] */
        let d = d + a.v[1] as u128 * a4
           + (a.v[2] as u128*2) * a.v[3] as u128;
        debug_assert!( d>>114==0 );
        /* [d t4+(tx<<48) t3 0 0 c] = [p8 0 0 p5 p4 p3 0 0 p0] */
        let u0 = d & M; 
        let d = d >> 52;
        ...
        debug_assert!( c>>114==0 );
        /* [d 0 t4 t3 0 c r0] = [p8 0 0 p5 p4 p3 0 p1 p0] */
        let d = d + a.v[2] as u128 * a4
           + a.v[3] as u128 * a.v[3] as u128;
        debug_assert!( d>>114==0 );
        /* [d 0 t4 t3 0 c r0] = [p8 0 p6 p5 p4 p3 0 p1 p0] */
        let c = c + (d & M) * R; 
        let d = d >> 52;
        ...
        let c = c + a0 * a.v[2] as u128
           + a.v[1] as u128 * a.v[1] as u128;
        debug_assert!( c>>114 == 0);
        /* [d 0 0 t4 t3 c r1 r0] = [p8 0 p6 p5 p4 p3 p2 p1 p0] */
    ...
    //     /* At most a single final reduction is needed; check if the value is >= the field characteristic */
        ...
        let x = (t4 >> 48) | 
            if (t4 == 0x0FFFFFFFFFFFF) & (m == 0xFFFFFFFFFFFFF)
                                & (t0 >= 0xFFFFEFFFFFC2F) { 1 } else { 0 };
...
//     /* Reduce t4 at the start so there will be at most a single carry from the first pass */
    ...
    fn from_bytes(b: &[u8]) -> FSecp256 {
        ...
        return if r4 == 0x0FFFFFFFFFFFF && (r3 & r2 & r1) == 0xFFFFFFFFFFFFF && r0 >= 0xFFFFEFFFFFC2F {
    ...
    //     secp256k1_fe_verify(a);
    // #endif
        if r.len()<32 { panic!("writing into a small buffer") }
