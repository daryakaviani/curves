...
impl<F, T> ECGroup<F, T> for P256<F, T> 
	...
	fn gen() -> Self {
		P256 {
		x: F::from_slice(&[0x2815B16F81798, 0xDB2DCE28D959F, 0xE870B07029BFC, 0xBBAC55A06295C, 0x79BE667EF9DC]),
		y: F::from_slice(&[0x7D08FFB10D4B8, 0x48A68554199C4, 0xE1108A8FD17B4, 0xC4655DA4FBFC0, 0x483ADA7726A3]),
		z: F::ONE,
		p: PhantomData,
		inf: false
		}
	...
	fn from_xy(x: &F, y: &F) -> Result<Self, &'static str> {
		...
		if lhs==rhs {
			Result::Ok(P256 { x: *x, y: *y, z: F::ONE, inf: false, p: PhantomData })
		} else {
	...
	fn op(a: &Self, b: &Self) -> Self  {
		...
		if b.inf {
			...
			return P256 { x: b.x, y: b.y, z: b.z, inf: b.inf, p: PhantomData }			
		}
	...
	fn neg(&self) -> Self {
		...
		y.normalize_weak();
		let y = y.neg(1);
		P256 { x:self.x, y: y, z:self.z, inf:self.inf, p: PhantomData }
	}


	fn scalar(&self, x: &T) -> Self	{
		if x.is_zero() || self.inf { return P256::INF; }
		...
		for i in (0..nb).rev() {
			...
			let mut db = P256 { x:r0.x, y:r0.y, z:r0.z, inf: r0.inf, p: PhantomData };
	...
	fn scalar_table_multi(table: &[Self], x:&T) -> Self {
		...
		for j in 0..nw {
			...
			if w!=0 {
				...
				dz[ w as usize ] = Self::op(&dz[w as usize], 
						&table[j*4 + p] );
			}
		}
	...
	fn from_bytes(b: &[u8]) -> Self {
		...
		P256 { x:x, y:y, z:F::ONE, inf: false, p: PhantomData }
	}
...
impl<F, T> P256<F, T> where F:Fq, T:Ford {
	...
	// //     secp256k1_fe_cmov(&r->z, &fe_1, a->infinity);
		...
		ry.mov(&b.y, a.inf);
		rz.mov(&F::ONE, a.inf);
		P256{ x: rx, y: ry, z: rz, inf: infinity, p: PhantomData }
	}
...
mod tests {
	...
	fn test_scalar() {
		...
		let ga = P256 { x: FSecp256::from_slice(&[0x13263f90ea75c, 0x2425e810119dd, 0xfe562feb0752, 0xdf86500f3f199, 0x4659812777be]),
	       y: FSecp256::from_slice(&[0x802233c467eb3, 0xa1a3b6fd9348d, 0x86a5417d0a357, 0xf29a5735a2950, 0x79e0f22c13c7]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);

		let a = FSecp256Ord::from_slice(&[0xc618f3ed, 0x0, 0x0, 0x0]);
		let ga = P256 { x: FSecp256::from_slice(&[0xed8e90c86e390, 0x9b3d78d9fed2d, 0xc72ec9749dd91, 0x592a27abeb755, 0x39bd595e528]),
	       y: FSecp256::from_slice(&[0xefd7780266439, 0xae7a93b9ff8d9, 0xf6011fd7740ac, 0x8af482a1648bb, 0xc53a52861dd]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData } ;
	    test(g,a,ga);

		let a = FSecp256Ord::from_slice(&[0x2dcba694033655c4, 0x2e, 0x0, 0x0]);
		let ga = P256 { x: FSecp256::from_slice(&[0x5ff5d1cd6de0e, 0x23a7f5254afd1, 0x838c6e21bc34a, 0x8c82c3fa058ce, 0xb93aa08b92e4]),
	       y: FSecp256::from_slice(&[0xc3578c5067efa, 0x17b8be93d58d4, 0x8745a1fc20b12, 0xc5fb244a8d49b, 0xce46028cdcc2]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData } ;
	    test(g,a,ga);

		let a = FSecp256Ord::from_slice(&[0x70e212955a161c1, 0xab62294f3829fcb7, 0x1, 0x0]);
		let ga = P256 { x: FSecp256::from_slice(&[0x20bd68d31dc85, 0x2e1a6729bf8c3, 0xa4dee0c643903, 0x49aa8c52d995f, 0x4494de998d64]),
	       y: FSecp256::from_slice(&[0x1c799615c9ebe, 0xefe4801766c0b, 0x88de11760be9d, 0x81aee3bcf20b6, 0x9a6ff21e213b]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);

	    let a = FSecp256Ord::from_slice(&[0xf3bc8e7468d31971, 0xb52d50b1711eb143, 0xb16e51ec1649c5b4, 0xe2]);
		let ga = P256 { x: FSecp256::from_slice(&[0x78277ef831227, 0xc748792170501, 0xaec56a20c3ab1, 0xec4fc50410aae, 0x72f1e506d223]),
	       y: FSecp256::from_slice(&[0x2e982b7f64695, 0x332e487292f1d, 0x78b1091ac1a6f, 0x40ca8c0b64fb4, 0xd703ffdf153b]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);


		let a = FSecp256Ord::from_slice( &[0x7bc42ecefd6bbefc, 0x866417a1d7f66cc4, 0x78d5683c92b5a85a, 0xfd22581fef3a5f4a] );
		let ga = P256 { x: FSecp256::from_slice( &[0x2dbccf2409831, 0xfcfadc00b85ea, 0x2c2181c2f96e0, 0x1a26f4fb3d9e0, 0x97edbf1b423e]),
	       y: FSecp256::from_slice( &[0x7e91ea68343d0, 0x6e8afa5b2e053, 0x924dbbcd3fa65, 0xcf2c0bc361664, 0x6b119fbe1c69]),
	       z: FSecp256::from_slice( &[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);


	    let a = FSecp256Ord::from_slice( &[0xb75db84fcf3100fa, 0x91b7d3f504137195, 0xdf77ea40442457c7, 0xd2942f7da864b8f7] );
		let ga = P256 { x: FSecp256::from_slice( &[0xce1fc14c40964, 0x878e5a24ba89b, 0x7b94afbe0497b, 0xc34d08ba41fe0, 0x8ed182cfabb3]),
	       y: FSecp256::from_slice( &[0xb7159d7e6174b, 0xe8eda690f07d, 0xddb2ab6e7d875, 0xf383bff267d94, 0xbb310a08591a]),
	       z: FSecp256::from_slice( &[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);

	    let a = FSecp256Ord::from_slice( &[0xf78541d24fe89bc7, 0x8a7007a0f47ac5b0, 0x9dce2824684f345f, 0x10ab8a41b7cd99b0] );
		let ga = P256 { x: FSecp256::from_slice( &[0x145fdb43a578a, 0xfd2e7448e6ab1, 0x753ac359ab721, 0x7ce216f629eb8, 0x7fd815a70e7a]),
	       y: FSecp256::from_slice( &[0x53e6edf689eb9, 0xff265e7abfbf6, 0xc0016cb2c92a8, 0x7e005c1e293f1, 0xf48a0781973c]),
	       z: FSecp256::from_slice( &[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(g,a,ga);
		...
		let a = FSecp256Ord::from_slice(&[0xb0691497e7bf4b99, 0xd9dcbf24f0445346, 0xce481f6dc2749af3, 0x1c]);
	    let h = P256 { x: FSecp256::from_slice(&[0xcf1af7b337e4e, 0xe12f2076cb56d, 0x45e71b7c5338a, 0xec368f90cf58, 0x3bd3f7d2c159]),
	       y: FSecp256::from_slice(&[0x6ae3949d01e47, 0x4a534948ca8ca, 0x7549d9835a27d, 0x7fc815495d549, 0xb83cdd607c90]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
		let ha = P256 { x: FSecp256::from_slice(&[0xce4ef839827db, 0x3089be0cd069d, 0x4a84319529969, 0xff5230b41e41c, 0x136f79d4dcff]),
	       y: FSecp256::from_slice(&[0x5542ba4ac45c0, 0xfb4ea43b01f60, 0x918d733a42f96, 0x505aac7f71176, 0x1bf29aee3bdf]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(h,a,ha);
		...
		let h = P256 { x: FSecp256::from_slice(&[0x1f9c9f527a236, 0xdd306f4eef9fa, 0x515a3a3c1c99d, 0x1d7adf97384d4, 0x52a3e8ff594f]),
	       ...
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    let a = FSecp256Ord::from_slice(&[0xbad5eb77a36f35bb, 0x6abe415244e51651, 0x1945e7cb89686466, 0xc6a1b0154b2c43db]);
		let ha = P256 { x: FSecp256::from_slice(&[0x228802496188d, 0x3bef55f2b071d, 0xb938461a56530, 0x874313449fe28, 0xbba7fea0f464]),
	       y: FSecp256::from_slice(&[0xf74fd3c55ab34, 0xd626a915fa8, 0x469d99767b837, 0x9c5cffa04c632, 0xb4ab605d0f52]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData }; 
	    test(h,a,ha);

		let h = P256 { x: FSecp256::from_slice(&[0x786c94789e82c, 0x9c4e03e271608, 0x7ff5c0e2a8eca, 0xd75e2a06f3c90, 0xaef122acbb3b]),
	       y: FSecp256::from_slice(&[0x2f1ecbf9c1685, 0xa43d471424995, 0xc5aa5c86a225c, 0xb8e9e329b8434, 0x7908afbd6254]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    let a = FSecp256Ord::from_slice(&[0x190e5240baf32821, 0x7143472686f0c572, 0x13f6fe3565b7e930, 0x1c6eab73e39330b9]);
		let ha = P256 { x: FSecp256::from_slice(&[0xec1fe348ca067, 0x8740c0030110, 0xad7d8de52f7d5, 0x963c6cae1a656, 0xdbfdb05db3cb]),
	       y: FSecp256::from_slice(&[0x26e561a144029, 0xe599b42febb03, 0xe467aa771caab, 0x90bb161d5a374, 0xcf7dec736205]),
	       z: FSecp256::from_slice(&[0x1, 0x0, 0x0, 0x0, 0x0]),
	       inf: false, p: PhantomData };
	    test(h,a,ha);
