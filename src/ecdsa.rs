...
pub fn ecdsa_verify_with_tables<F: Fq, T: Ford, E: ECGroup<F,T>>(msg: &[u8], sig: (&T, &T), gentable: &[E], pktable: &[E] ) -> bool {
	...
	// p = g^u1 * pk^u2
	// check p.x == r
	let p = E::op( &E::scalar_table_multi(pktable, &u2), &E::scalar_table_multi(gentable, &u1) ).affine();
	let mut pb = [0;32];
	p.x().to_bytes(&mut pb);
...
mod tests {
	...
	fn test_ecdsa() {
	    ...
	    let (rx,s) = ecdsa_sign::<FSecp256, FSecp256Ord, P256<FSecp256, FSecp256Ord>>(msg, &sk, &mut rng);
	...
	fn test_sign() {
		let mut rng = rand::os::OsRng::new().unwrap();
		let pk = P256::from_xy( 
			&FSecp256::from_slice(&[0x7af580e1c312a, 0xd6836084d8c4e, 0x5052dc1851870, 0xafe55d61c400, 0x8d7b1eb00714]),
	       	&FSecp256::from_slice(&[0x58828dca1f64e, 0xcf1ed800b91e9, 0x4de0a50ffa76e, 0xdd6b840fa7b97, 0xb261db36aebe]) 
	   	).unwrap();

	    let sk = FSecp256Ord::from_slice(&[0x2944b11bef5b2ec9, 0xdeedd1bda9f8eacd, 0x5c1f888b21b87605, 0x31c615aca4a906c6]);

		let msg = &"this is a test".as_bytes();

	    let (rx,s): (FSecp256Ord, FSecp256Ord) = ecdsa_sign::<FSecp256, FSecp256Ord, P256<FSecp256, FSecp256Ord>>(msg, &sk, &mut rng);
		println!(" r = {:?}", rx);
		println!(" s = {:?}", s);
